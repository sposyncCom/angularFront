...
declare const deviceControl:any;


@Component({
  selector: 'app-p2p-lab',
  templateUrl: './p2p-lab.component.html',
  styleUrls: ['./p2p-lab.component.css']
}) 

export class P2pLabComponent implements OnInit {
  ...
  @ViewChild('classButton') classButton: ElementRef;
  ...
  ngOnInit(): void {
      ...
      this.infoWinApi.p2pLabGet(thisObuser).subscribe({
          ...
          const updateDom = new Observable((observer) => { 
            ...
            console.log('let position = updateStatusSource():', position);
          ...
          this.router.navigateByUrl('/p2p-lab'); 
  ...
  callClick(varslug:string){
    ...
    console.log(' SUBMIT CLICK ', varslug );
    const thisObuser = this.infoWinApi.obuser;
    const apiData = {
        eventData :  varslug,
        userName : thisObuser
    ...
    this.infoWinApi.p2pLabPost(apiData).subscribe({
